...
### Changed
- **BREAKING:** `list_concepts` MCP tool now returns `{ concepts: Concept[], cursor?: string }` instead of a bare `Concept[]` array, and accepts new `cursor` and `limit` inputs. Pagination is cursor-based with a stable `ORDER BY slug ASC` tiebreaker. Default `limit` is 50; hard cap is 500. Callers that previously iterated `response` as an array must now destructure: `const { concepts } = response`. Refs #50 (PR 6 of the context-efficiency rollout).
